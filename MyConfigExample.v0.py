...
if configMgr.readFromTree:
    ...
    fhc_fgd1_sig_files.append("samples/ldm/DPhoton_Production_BdNMC_Oct_2020_Pi0_decay_parent_FGD1.root")
    #
    ...
    fhc_fgd2_sig_files.append("samples/ldm/DPhoton_Production_BdNMC_Oct_2020_Pi0_decay_parent_FGD2.root")
...
if configMgr.readFromTree:
    ...
    for sam in [topSample, wzSample, bgSample]:
            ...
            if sam==topSample:
                sam.weights = ("1","weight_corr_total",str(pot_data/pot),"(nuElectronElasticTopo==10)")
            elif sam==wzSample:
                sam.weights = ("1","weight_corr_total",str(pot_data/pot),"(nuElectronElasticTopo==1)")            
            else:
                sam.weights = ("1","weight_corr_total",str(pot_data/pot),"!(nuElectronElasticTopo==10 || nuElectronElasticTopo==1)")            
            print(sam.name,pot,sam.weights)
            raw_input("Press Enter to continue.")
            
            if myFitType==FitType.Exclusion:
                for sig in sigSamples:
                    sigSample.setFileList(fhc_fgd1_sig_files)
                    sigSample.weights = ("1","weight_corr_total")
...
#-------------------------------------------------
mm2J = bkt.addChannel("selelec_mom",["SR1"],1,0.,1200.)
mm2J.useOverflowBin=True
#bkt.setValidationChannels([mm2J])
...
if myFitType==FitType.Background:
     bkt.setBkgConstrainChannels([mm2J])
     if doValidation:
         bkt.setValidationChannels([costheta1,emom2,costheta2])
elif myFitType==FitType.Exclusion:
     bkt.setSignalChannels([mm2J])
     if doValidation:
         bkt.setValidationChannels([costheta1,emom2,costheta2])
